...
final class ImportProcess extends Location\Util\Process
{
	...
	public function onBeforePerformIteration()
	{
		...
		if(!$this->data['inited'])
		{
			...
			if((string) $this->data['LOCAL_PATH'] === '')
				$this->data['LOCAL_PATH'] = \CTempFile::GetDirectoryName(10);
	...
	protected function stageDownloadFiles()
	{
		if($this->checkSource(self::SOURCE_FILE)) // user uploaded file
		{
			$this->data['files'] = array(
				array(
					'size' => filesize($this->data['LOCAL_PATH'].self::getFileNameByIndex(0)),
					'memgroup' => 'static'
				)
			);
			$this->nextStage();
	...
	public function getRemoteLayout($getFlat = false)
	{
		...
		if(\Bitrix\Main\IO\File::isFileExists($localPath.self::LOCAL_LAYOUT_FILE))
		{
			try
			{
				\Bitrix\Main\IO\File::deleteFile($localPath.self::LOCAL_LAYOUT_FILE);
	...
	public function getRemoteTypes()
	{
		if(!$this->useCache || !isset($this->data['settings']['remote']['types']))
		{
			...
			if(\Bitrix\Main\IO\File::isFileExists($localPath.self::LOCAL_TYPE_FILE))
			{
				try
				{
					\Bitrix\Main\IO\File::deleteFile($localPath.self::LOCAL_TYPE_FILE);
					\Bitrix\Main\IO\Directory::deleteDirectory($localPath);
				}
				catch(\Exception $e)
				{
	...
	public function getRemoteExternalServices()
	{
		if(!$this->useCache || !isset($this->data['settings']['remote']['external_services']))
		{
			...
			if(\Bitrix\Main\IO\File::isFileExists($localPath.self::LOCAL_EXTERNAL_SERVICE_FILE))
			{
				try
				{
					\Bitrix\Main\IO\File::deleteFile($localPath.self::LOCAL_EXTERNAL_SERVICE_FILE);
					\Bitrix\Main\IO\Directory::deleteDirectory($localPath);
				}
				catch(\Exception $e)
				{
	...
	*/
	...
	public function saveUserFile($inputName)
	{
		if(is_array($_FILES[$inputName]))
		{
			...
			if(!copy($_FILES[$inputName]['tmp_name'], $this->data['LOCAL_PATH'].self::getFileNameByIndex(0)))
			{
