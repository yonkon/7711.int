...
class CSofteffectXlsExcel {
	private $_file = "";
	private $_reader = null;
	private $_charset = 'cp1251';

	public function CSofteffectXlsExcel($file, $XLS_FIRST_ROW=1, $chunkSize=100) {
		...
		if (!$reader->canRead($this->_file)) {
			...
			if (!$reader->canRead($this->_file)) {
				throw new Exception(GetMessage('SOFTEFFECT_XLS_ERROR_FORMAT'));
			}
		}
