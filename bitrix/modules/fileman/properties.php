...
class CIBlockPropertyMapGoogle extends CIBlockPropertyMapInterface
{
	...
	function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
	{
		...
		if(isset($GLOBALS['googleMapLastNumber']) && $arProperty['MULTIPLE'] == 'Y')
	...
	<?else:?>
		function setPointValue_<?echo $MAP_ID?>(obPoint)
		{
			...
			if(window[updPP])
			{
				...
				google.maps.event.addListener(window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_, "dragend", updatePointPosition_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_);
...
?>
<input type="text" style="width:125px;margin:0 0 4px" name="point_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_lat" id="point_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_lat" onchange="setInputPointValue_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_()" />, <input type="text" style="width:125px;margin:0 15px 4px 0;" name="point_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_lon" id="point_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_lon" onchange="setInputPointValue_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_()" />
<div id="point_control_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_" style="display:none;margin:0 0 4px"><a href="javascript:void(0);" onclick="findPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_()"><?echo GetMessage('IBLOCK_PROP_MAP_GOOGLE_GOTO_POINT')?></a> | <a href="javascript:void(0);" onclick="if (confirm('<?echo CUtil::JSEscape(GetMessage('IBLOCK_PROP_MAP_GOOGLE_REMOVE_POINT_CONFIRM'))?>')) removePoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_()"><?echo GetMessage('IBLOCK_PROP_MAP_GOOGLE_REMOVE_POINT')?></a></div><br />
<input type="text" style="display:none;" id="value_<?echo $MAP_ID;?>__n<?=$GLOBALS['googleMapLastNumber']?>_" name="<?=htmlspecialcharsbx($strHTMLControlName["VALUE"])?>" value="<?=htmlspecialcharsEx($value["VALUE"])?>" />
<script>
	...
	function BXWaitForMap_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_()
	{
		if (!window.GLOBAL_arMapObjects['<?echo $MAP_ID?>'])
			setTimeout(BXWaitForMap_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_, 300);
		else
		{
			if(!window.markersBounds_<?echo $MAP_ID?>)
				window.markersBounds_<?echo $MAP_ID?> = new google.maps.LatLngBounds();
			window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_ = null;
			...
			BX('point_control_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_').style.display = 'none';

			<?if ($bHasValue):?>
				setPointValue_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_({latLng: new google.maps.LatLng(<?echo $POINT_LAT?>, <?echo $POINT_LON?>)});
				window.markersBounds_<?echo $MAP_ID?>.extend(new google.maps.LatLng(<?echo $POINT_LAT?>, <?echo $POINT_LON?>));
				if (<?=$GLOBALS['googleMapLastNumber']?> > 0)
...
function findPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_()
{
	if (null != window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_)
		window.GLOBAL_arMapObjects['<?echo $MAP_ID?>'].panTo(window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_.getPosition());
}

function removePoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_()
{
	window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_.setMap(null);
	window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_ = null;

	BX('point_control_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_').style.display = 'none';

	updatePointPosition_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_();

	updateMapHint_<?echo $MAP_ID?>();
}

function setPointValue_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_(obEvent)
{
	...
	if (null == window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_)
	{
		window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_ = new google.maps.Marker({
			position: obPoint.latLng,
			map: window.GLOBAL_arMapObjects['<?echo $MAP_ID?>'],
			draggable:true
		});
	...
	{
		window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_.setPosition(obPoint.latLng);
	}

	BX('point_control_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_').style.display = 'inline-block';
	BX('bx_map_hint_novalue_<?echo $MAP_ID?>').style.display = 'none';
	BX('bx_map_hint_value_<?echo $MAP_ID?>').style.display = 'block';

	updatePointPosition_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_(obPoint);
	window.GLOBAL_arMapObjects['<?echo $MAP_ID?>'].panTo(window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_.getPosition());
}

function setInputPointValue_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_()
{
	var vv = [BX('point_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_lat').value, BX('point_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_lon').value];
	if (vv[0] == '' && vv[1] == '')
	{
		removePoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_();
	}
	var v = [parseFloat(BX('point_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_lat').value), parseFloat(BX('point_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_lon').value)];
	if (!isNaN(v[0]) && !isNaN(v[1]))
	{
		setPointValue_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_(v);
	}
}

function updatePointPosition_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_(obPoint)
{
	var obP = [];
	if (!!obPoint && !!obPoint.latLng)
		obP.push(obPoint.latLng.lat(), obPoint.latLng.lng());
	else if (!!window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_)
		obP.push(window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_.latLng.lat(), window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_.latLng.lng());
	else
		obP = null;
	var obInput = BX('value_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_');
	obInput.value = null == obP ? '' : obP[0] + ',' + obP[1];
	BX('point_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_lat').value = obP ? obP[0] : '';
	BX('point_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_lon').value = obP ? obP[1] : '';
}

	BX.ready(setTimeout(BXWaitForMap_<?echo $MAP_ID?>__n<?=$GLOBALS['googleMapLastNumber']?>_, 1));
...
<?
	...
	if ($arProperty['MULTIPLE'] == 'N' && isset($GLOBALS['googleMapLastNumber']))
		unset($GLOBALS['googleMapLastNumber']);
...
<?
	...
	function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
	{
		...
		if(isset($GLOBALS['yandexMapLastNumber']) && $arProperty['MULTIPLE'] == 'Y')
			$GLOBALS['yandexMapLastNumber']++;
		else
			$GLOBALS['yandexMapLastNumber']=0;
...
?>
<input type="text" style="width:125px;margin:0 0 4px" name="point_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_lat" id="point_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_lat" onchange="setInputPointValue_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_()" />, <input type="text" style="width:125px;margin:0 15px 4px 0;" name="point_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_lon" id="point_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_lon" onchange="setInputPointValue_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_()" />
<div id="point_control_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_" style="display:none;margin:0 0 4px"><a href="javascript:void(0);" onclick="findPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_()"><?echo GetMessage('IBLOCK_PROP_MAP_YANDEX_GOTO_POINT')?></a> | <a href="javascript:void(0);" onclick="if (confirm('<?echo CUtil::JSEscape(GetMessage('IBLOCK_PROP_MAP_YANDEX_REMOVE_POINT_CONFIRM'))?>')) removePoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_()"><?echo GetMessage('IBLOCK_PROP_MAP_YANDEX_REMOVE_POINT')?></a></div><br />
<input type="text" style="display:none;" id="value_<?echo $MAP_ID;?>__n<?=$GLOBALS['yandexMapLastNumber']?>_" name="<?=htmlspecialcharsbx($strHTMLControlName["VALUE"])?>" value="<?=htmlspecialcharsEx($value["VALUE"])?>" />
<script>
	...
	function BXWaitForMap_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_()
	{
		if (!window.GLOBAL_arMapObjects['<?echo $MAP_ID?>'])
			setTimeout(BXWaitForMap_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_, 300);
		else
		{
			window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_ = null;
			...
			BX('point_control_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_').style.display = 'none';

			<?if ($bHasValue):?>
				setPointValue_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_([<?echo $POINT_LAT?>, <?echo $POINT_LON?>]);
				if (<?=$GLOBALS['yandexMapLastNumber']?> > 0)
	...
	function findPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_()
	{
		if (null != window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_)
		{
			window.setDefaultPreset_<?echo $MAP_ID?>();
			window.GLOBAL_arMapObjects['<?echo $MAP_ID?>'].panTo(window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_.geometry.getCoordinates(),{delay:0});
			window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_.options.set({preset: 'twirl#redIcon'});

		}
	}

	function removePoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_()
	{
		window.GLOBAL_arMapObjects['<?echo $MAP_ID?>'].geoObjects.remove(window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_);
		window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_ = null;

		BX('point_control_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_').style.display = 'none';

		updatePointPosition_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_();

		updateMapHint_<?echo $MAP_ID?>();
	}

	function setPointValue_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_(obEvent)
	{
		var obPoint = BX.type.isArray(obEvent) ? obEvent : obEvent.get("coordPosition");

		if (null == window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_)
		{
			window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_ = new ymaps.Placemark(obPoint, {}, {draggable:true});
			window.GLOBAL_arMapObjects['<?echo $MAP_ID?>'].geoObjects.add(window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_);
			window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_.events.add('dragend', updatePointPosition_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_);
		}
		else
		{
			window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_.geometry.setCoordinates(obPoint);
		}

		BX('point_control_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_').style.display = 'inline-block';
		BX('bx_map_hint_novalue_<?echo $MAP_ID?>').style.display = 'none';
		BX('bx_map_hint_value_<?echo $MAP_ID?>').style.display = 'block';

		updatePointPosition_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_(obPoint);
		window.GLOBAL_arMapObjects['<?echo $MAP_ID?>'].panTo(obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_.geometry.getCoordinates(),{delay:0});
	}

	function setInputPointValue_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_()
	{
		var vv = [BX('point_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_lat').value, BX('point_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_lon').value];
		if (vv[0] == '' && vv[1] == '')
		{
			removePoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_();
		}
		var v = [parseFloat(BX('point_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_lat').value), parseFloat(BX('point_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_lon').value)];
		if (!isNaN(v[0]) && !isNaN(v[1]))
		{
			setPointValue_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_(v);
		}
	}

	function updatePointPosition_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_(obPoint)
	{
		//var obPosition = obPoint.getGeoPoint();
		if (!!obPoint && !!obPoint.geometry)
			obPoint = obPoint.geometry.getCoordinates();
		else if (!!window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_)
			obPoint = window.obPoint_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_.geometry.getCoordinates();
		else
			obPoint = null;

		var obInput = BX('value_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_');
		obInput.value = null == obPoint ? '' : obPoint[0] + ',' + obPoint[1];

		BX('point_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_lat').value = obPoint ? obPoint[0] : '';
		BX('point_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_lon').value = obPoint ? obPoint[1] : '';
	}

	BX.ready(setTimeout(BXWaitForMap_<?echo $MAP_ID?>__n<?=$GLOBALS['yandexMapLastNumber']?>_, 1));
...
<?
		...
		if ($arProperty['MULTIPLE'] == 'N' && isset($GLOBALS['yandexMapLastNumber']))
			unset($GLOBALS['yandexMapLastNumber']);
